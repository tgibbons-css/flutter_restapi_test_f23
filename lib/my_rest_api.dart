...
class MyRestAPI{
  ...
  Future<List<CatFact>>  fetchCatFact() async {
    final response = await http
        .get(Uri.parse('https://cat-fact.herokuapp.com/facts/random?animal_type=cat&amount=25'));

    if (response.statusCode == 200) {
      // If the server did return a 200 OK response,
      // then parse the JSON.
      return catFactFromJson(jsonDecode(response.body));
    } else {
      // If the server did not return a 200 OK response,
      // then throw an exception.
      throw Exception('Failed to load album');
  ...
  Future<List<PokemonCard>>  fetchPokemon() async {
    ...
    if (response.statusCode == 200) {
      // If the server did return a 200 OK response,
      // then parse the JSON.
      CardList list = cardListFromJson(jsonDecode(response.body));
